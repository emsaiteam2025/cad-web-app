...
function renderElements(elements) {
    elements.forEach(item => {
        ...
        if (item.type === 'pipe') {
            ...
            let arc = new fabric.Circle({ radius: 10, startAngle: -Math.PI/2, endAngle: Math.PI/2, fill: 'transparent', stroke: '#0056b3', strokeWidth: 3, originX: 'center', originY: 'center', left: 15 });
            obj = new fabric.Group([f1, l1, arc], { left: item.x - 20, top: item.y - 15, customType: 'short_a' });
            ...
            obj = new fabric.Group([p], { left: item.x - 15, top: item.y - 15, customType: 'reducer' });
